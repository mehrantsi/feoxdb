...
impl SectorBuffer {
    pub fn new(sector: u64) -> Result<Self> {
        let mut buffer = AlignedBuffer::new(FEOX_BLOCK_SIZE)?;
        buffer.set_len(FEOX_BLOCK_SIZE);

        let header = buffer.as_mut_slice();
        header[0..2].copy_from_slice(&SECTOR_MARKER.to_le_bytes());
    ...
    pub fn write_data(&mut self, offset: usize, data: &[u8]) -> usize {
        ...
        if offset >= FEOX_BLOCK_SIZE {
            return 0;
        }

        let available = FEOX_BLOCK_SIZE - offset;
        let to_write = data.len().min(available);

        if to_write > 0 {
            self.data.as_mut_slice()[offset..offset + to_write].copy_from_slice(&data[..to_write]);
        }
        to_write
    }
    ...
    pub fn set_seq_number(&mut self, seq: u16) {
        self.seq_number = seq;
        self.data.as_mut_slice()[2..4].copy_from_slice(&seq.to_le_bytes());
    }
