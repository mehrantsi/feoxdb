...
fn test_update_existing_key() {
    ...
    store.insert(key, value1, None).unwrap();
    assert_eq!(store.get(key).unwrap(), value1);

    store.insert(key, value2, None).unwrap();
...
fn test_empty_store() {
    ...
    store.insert(b"key", b"value", None).unwrap();
    assert!(!store.is_empty());
...
fn test_timestamp_conflict_resolution() {
    ...
    store.insert(key, b"v1", Some(100)).unwrap();

    // Update with higher timestamp succeeds
    store.insert(key, b"v2", Some(200)).unwrap();
    assert_eq!(store.get(key).unwrap(), b"v2");

    // Update with lower timestamp fails
    let result = store.insert(key, b"v3", Some(150));
    assert!(result.is_err());
...
fn test_delete_with_timestamp() {
    ...
    store.insert(key, b"value", Some(100)).unwrap();

    // Delete with older timestamp fails
    let result = store.delete(key, Some(50));
    assert!(result.is_err());
    assert!(matches!(result.unwrap_err(), FeoxError::OlderTimestamp));
    ...
    store.delete(key, Some(200)).unwrap();
    assert!(!store.contains_key(key));
}
...
fn test_memory_limit_enforcement() {
    ...
    store.insert(b"k1", b"v1", None).unwrap();

    // Try to insert a large value that exceeds limit
    let large_value = vec![0u8; 2048];
    let result = store.insert(b"k2", &large_value, None);
    assert!(result.is_err());
...
fn test_memory_usage_tracking() {
    ...
    for i in 0..10 {
        ...
        store
            .insert(key.as_bytes(), value.as_bytes(), None)
            .unwrap();
    }
    ...
    for i in 0..5 {
        let key = format!("key_{}", i);
        store.delete(key.as_bytes(), None).unwrap();
    }
...
fn test_range_query_inclusive() {
    ...
    // Insert sorted keys
    store.insert(b"key:001", b"val1", None).unwrap();
    store.insert(b"key:002", b"val2", None).unwrap();
    store.insert(b"key:003", b"val3", None).unwrap();
    store.insert(b"key:004", b"val4", None).unwrap();
    store.insert(b"key:005", b"val5", None).unwrap();
...
fn test_atomic_increment_basic() {
    ...
    // Initialize counter
    let zero: i64 = 0;
    store.insert(key, &zero.to_le_bytes(), None).unwrap();

    // Increment
    let val = store.atomic_increment(key, 1, None).unwrap();
    assert_eq!(val, 1);

    let val = store.atomic_increment(key, 5, None).unwrap();
    assert_eq!(val, 6);

    // Decrement
    let val = store.atomic_increment(key, -2, None).unwrap();
    assert_eq!(val, 4);
}
...
fn test_atomic_increment_create_if_not_exists() {
    ...
    let val = store.atomic_increment(b"new_counter", 100, None).unwrap();
    assert_eq!(val, 100);

    let val = store.atomic_increment(b"new_counter", 50, None).unwrap();
    assert_eq!(val, 150);
}
...
fn test_atomic_increment_invalid_value() {
    ...
    store.insert(b"not_a_counter", b"text", None).unwrap();

    // Try to increment
    let result = store.atomic_increment(b"not_a_counter", 1, None);
    assert!(result.is_err());
...
fn test_atomic_increment_saturation() {
    ...
    store.insert(key, &max.to_le_bytes(), None).unwrap();

    // Increment should saturate, not overflow
    let val = store.atomic_increment(key, 10, None).unwrap();
...
fn test_json_patch_basic() {
    ...
    store.insert(key, doc, None).unwrap();

    let patch = br#"[{"op":"replace","path":"/age","value":31}]"#;
    store.json_patch(key, patch, None).unwrap();
...
fn test_json_patch_on_non_json() {
    ...
    store.insert(b"not_json", b"plain text", None).unwrap();

    let patch = br#"[{"op":"add","path":"/foo","value":"bar"}]"#;
    let result = store.json_patch(b"not_json", patch, None);
    assert!(result.is_err());
}
...
fn test_concurrent_mixed_operations() {
    ...
    for i in 0..100 {
        let key = format!("key_{}", i);
        store.insert(key.as_bytes(), b"initial", None).unwrap();
    }
    ...
    for t in 0..5 {
        ...
        handles.push(thread::spawn(move || {
            for i in 0..100 {
                ...
                let _ = store_clone.insert(key.as_bytes(), value.as_bytes(), None);
            }
        }));
    }
    ...
    for _ in 0..2 {
        ...
        handles.push(thread::spawn(move || {
            for i in 90..100 {
                let key = format!("key_{}", i);
                let _ = store_clone.delete(key.as_bytes(), None);
            }
        }));
    }
...
fn test_concurrent_atomic_increments() {
    ...
    for _ in 0..10 {
        ...
        handles.push(thread::spawn(move || {
            for _ in 0..100 {
                store_clone.atomic_increment(key, 1, None).unwrap();
            }
        }));
    }
    ...
    let final_value = store.atomic_increment(key, 0, None).unwrap();
...
fn test_empty_key_rejected() {
    ...
    let result = store.insert(b"", b"value", None);
    assert!(result.is_err());
...
fn test_empty_value_rejected() {
    ...
    let result = store.insert(b"key", b"", None);
    assert!(result.is_err());
...
fn test_builder_configuration() {
    ...
    store.insert(b"test", b"value", None).unwrap();
