...
fn test_basic_persistence() {
    ...
    // Create store and insert data
    {
        ...
        store
            .insert(b"persist_key", b"persist_value", None)
            .unwrap();
        store
            .insert(b"another_key", b"another_value", None)
            .unwrap();
...
fn test_value_offloading() {
    ...
    store.insert(b"large_key", &large_value, None).unwrap();
...
fn test_delete_persistence() {
    ...
    // Insert and delete
    {
        ...
        store.insert(b"del_key1", b"value1", None).unwrap();
        store.insert(b"del_key2", b"value2", None).unwrap();
        store.insert(b"keep_key", b"keep_value", None).unwrap();

        store.delete(b"del_key1", None).unwrap();
        store.delete(b"del_key2", None).unwrap();
...
fn test_update_persistence() {
    ...
    // Multiple updates
    {
        ...
        store.insert(b"update_key", b"value1", None).unwrap();
        store.insert(b"update_key", b"value2", None).unwrap();
        store.insert(b"update_key", b"value3", None).unwrap();
        store.insert(b"update_key", b"final_value", None).unwrap();
...
fn test_atomic_increment_persistence() {
    ...
    // Perform atomic operations
    {
        ...
        let zero: i64 = 0;
        store.insert(b"counter", &zero.to_le_bytes(), None).unwrap();

        for _ in 0..100 {
            store.atomic_increment(b"counter", 1, None).unwrap();
        }
    ...
    // Verify counter value persisted
    {
        ...
        let value = store.atomic_increment(b"counter", 0, None).unwrap();
        assert_eq!(value, 100);
    }
}
